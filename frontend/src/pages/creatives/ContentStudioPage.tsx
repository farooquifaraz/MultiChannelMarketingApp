...
import {
  ...
  X, Users, CheckCircle2, ExternalLink,
} from 'lucide-react';
...
import type { ContactGroupDto } from '../../types/contact.types';
...
export default function ContentStudioPage({ embedded = false, onSendToBanner }: { embedded?: boolean; onSendToBanner?: (prompt: string) => void } = {}) {
  ...
  const [campaignModal, setCampaignModal] = useState<ChannelId | null>(null);
  ...
  // Open the in-place campaign modal — NO navigation, so all Studio work is preserved.
  const openCampaign = (c: ChannelId) => {
  ...
  return (
    <div className="space-y-5" onClick={() => combineOpen && setCombineOpen(false)}>
      ...
      {campaignModal && (
        <CampaignModal
          channel={campaignModal}
          defaultName={(name || brief).trim().slice(0, 50) || `${CHANNEL_META[campaignModal].label} campaign`}
...
/**
 * Create a campaign from a locked channel WITHOUT leaving Creative Studio.
 * Saves the template, picks a contact group, then creates a draft (or sends/schedules).
 * The Studio stays mounted underneath, so nothing the user did is lost.
 */
function CampaignModal({ channel, defaultName, ensureSaved, onClose, navigate }: {
  channel: ChannelId;
  defaultName: string;
  ensureSaved: (c: ChannelId) => Promise<string | undefined>;
  onClose: () => void;
  navigate: (to: string) => void;
}) {
  const meta = CHANNEL_META[channel];
  const [cname, setCname] = useState(defaultName);
  ...
  const loadGroups = () => {
    ...
    contactApi.getGroups().then((r: any) => { const g: ContactGroupDto[] = r.data || []; setGroups(g); if (g[0] && !groupId) setGroupId(g[0].id); })
      .catch(() => {}).finally(() => setLoadingGroups(false));
  };
  useEffect(loadGroups, []); // eslint-disable-line

  const create = async (send: boolean) => {
    if (!groupId) { toast.error('Pick a contact group'); return; }
    if (!cname.trim()) { toast.error('Name the campaign'); return; }
    setBusy(send ? 'send' : 'draft');
    try {
      const templateId = await ensureSaved(channel);
      const res: any = await campaignApi.create({ name: cname.trim(), channel, templateId, groupId } as any);
      const id = res.data?.id;
      if (send && id) await campaignApi.send(id, schedule || undefined);
      setCreatedId(id);
      toast.success(send ? (schedule ? 'Campaign scheduled' : 'Campaign sending now') : 'Campaign saved as draft');
    } catch (err: any) { toast.error(err?.response?.data?.message || 'Could not create the campaign'); }
    finally { setBusy(''); }
  };
  ...
  return (
    <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-50 p-4" onClick={(e) => e.target === e.currentTarget && onClose()}>
      <div className="bg-white rounded-2xl w-full max-w-md shadow-2xl overflow-hidden">
        <div className={`flex items-center gap-3 px-5 py-4 text-white bg-gradient-to-r ${meta.head}`}>
          <span className="bg-white/90 rounded p-1"><ChannelLogo channel={channel} size={18} /></span>
          <b className="text-base">{meta.label} campaign</b>
        ...
        {createdId ? (
          <div className="p-6 text-center space-y-4">
            ...
            <div>
              ...
              <p className="text-sm text-gray-500 mt-1">Your Creative Studio work is still here — nothing was lost.</p>
            </div>
          ...
          <div className="p-5 space-y-4">
            ...
            <p className="text-[11px] text-gray-400 text-center">This creates the campaign in the background — you stay right here.</p>
          </div>
        )}
      </div>
    </div>
